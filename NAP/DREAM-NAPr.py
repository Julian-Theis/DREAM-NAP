...
from keras.layers import Input, LSTM, Dense, concatenate, Dropout, RepeatVector
from keras.models import Model
import numpy as np
import pandas as pd
...
if __name__ == "__main__":
    ...
    for fold in range(n_folds):
        ...
        # Resource Inputs
        # res_inputs = Input(shape=(res_insize,))
        # res_l1 = Dense(70, activation='relu')(res_inputs)

        # Actual Classifier:
        class_inputs = Input(shape=(insize,))
        ae_x = Dense(250, activation='relu')(class_inputs)
        bn1 = BatchNormalization()(ae_x)
        # l1 = concatenate([ae_x, res_l1])
        l1 = Dense(200, activation='relu', name="classifier_l1")(bn1)
        bn2 = BatchNormalization()(l1)
        l2 = Dropout(dropout_rate)(bn2)
        l2 = Dense(150, activation='relu', name="classifier_l2")(l2)
        bn3 = BatchNormalization()(l2)
        l2 = Dropout(dropout_rate)(bn3)
        l3 = Dense(100, activation='relu', name="classifier_l3")(l2)
        ...
        with open(str("results/" + benchmark + "/" + benchmark + "_" + str(fold) + "_results_napr.json"), 'w') as outfile:

...
if __name__ == "__main__":
    ...
    for fold in range(n_folds):
        ...
        # CREATE MODEL
        model = Sequential()
        model.add(Dense(insize, input_dim=insize, activation='sigmoid'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 1.2), activation='sigmoid'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.6), activation='sigmoid'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.3), activation='sigmoid'))
        model.add(Dropout(dropout_rate))
        ...
        with open(str("results/" + benchmark + "/" + benchmark + "_" + str(fold) + "_results_nap-sigm.json"), 'w') as outfile:

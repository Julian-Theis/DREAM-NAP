...
if __name__ == "__main__":
    ...
    for fold in range(n_folds):
        ...
        ## CLASS WEIGHTS
        #y_integers = train_integer_encoded.flatten()
        #for y_int in y_integers:
            #hotencod = onehot_encoder.transform([[y_int]])
            #print(hotencod)
            #maxx =  np.argmax(hotencod)
            #print(maxx)
            #print("True integer value: ", y_int, " --- Hot encoded is ", maxx)

        y_integers = train_integer_encoded.flatten()
        y_unique, y_counts = np.unique(y_integers, return_counts=True)
        print(y_counts)
        d_class_weights = {}
        for i in range(len(np.unique(y_integers))):
            key = np.unique(y_integers)[i]
            d_class_weights[key] = (y_counts[i])
        print(d_class_weights)

        #{0: 3733, 1: 44, 2: 95, 3: 13, 4: 5, 5: 3732, 6: 4, 7: 3865, 8: 874}
        d_class_weights = {0: 1, 1: 10, 2: 10, 3: 10, 4: 10, 5: 1, 6: 10, 7: 1, 8: 5}
        ...
        #for i in range(len(np.unique(y_integers))):
        #    key = np.unique(y_integers)[i]
        #    d_class_weights[key] = class_weights[i]
        ...
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.9), activation='relu'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.9), activation='relu'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.8), activation='relu'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.6), activation='relu'))
        model.add(Dropout(dropout_rate))
        model.add(Dense(int(insize * 0.4), activation='relu'))
        model.add(Dropout(dropout_rate))
        ...
        hist = model.fit([X_train], [Y_train], batch_size=n_batch_size, epochs=n_epochs, shuffle=True,
                         ...
                         callbacks=[TestCallbackNew(X_test, Y_test, Y_test_int)], class_weight=d_class_weights)

        with open(str("results/" + benchmark + "/" + benchmark + "_" + str(fold) + "_results_nap_weighted.json"), 'w') as outfile:

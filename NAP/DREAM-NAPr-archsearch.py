...
def performTest(arch, benchmark, arch_num):
    ...
    layer = Dense(arch[0], activation='relu')(class_inputs)
    layer = BatchNormalization()(layer)

    for i in range(len(arch)):
        if i != 0:
            layer = Dense(arch[i], activation='relu', name="classifier_l" + str(i))(layer)
            layer = BatchNormalization()(layer)
    ...
    with open(str("results/" + benchmark + "/" + benchmark + "_" + str(arch_num) + "_results_napr-archsearch_" + str(h_layers[0]) + ".json"), 'w') as outfile:
...
if __name__ == "__main__":
    ...
    outF = open("results/" + benchmark + "/" + benchmark + "_architectures_to_" + str(h_layers[0]) + "_napr.csv", "w")
    for line in arches:
        outF.write(str(line))
        outF.write("\n")
    outF.close()
